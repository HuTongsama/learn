...
static void test_move()
{
	...
	EXPECT_EQ_TRUE(lept_is_equal(&v3, &v1));
	lept_free(&v1);
	lept_free(&v2);
	lept_free(&v3);
}
...
static void test_access_array()
{
	...
	for (j = 0; j <= 5; j += 5)
	{
		...
		EXPECT_EQ_TRUE(lept_get_array_capacity(&a) > 8);
		lept_shrink_array(&a);
...
static void test_access_object()
{
	...
	for (j = 0; j <= 5; j += 5)
	{
		...
		for (i = 0; i < 10; i++)
		{
			...
			EXPECT_EQ_TRUE(index != LEPT_KEY_NOT_EXIST);
	...
	EXPECT_EQ_TRUE(lept_get_object_capacity(&o) > 8);
